...
parcoursArbre(J,Pmax,R,Value):- assert(maximizer(J)), assert(joueurCourant(J)), parcours(1,1,Pmax,[1,0]), parcours(2,1,Pmax,[2,0]), feuille([1,0],X1), feuille([2,0],X2), coupAJouerMaximizer([X1,X2],R,Value),!.

parcours(X, P, Pmax, L):- P==Pmax, nl, write('feuille'), print(L), evaluate(Value), assert(feuille(L, Value)).
...
evaluate(X):- X is random(10).

choixValeurNoeud(L,R,Value):- joueurCourant(X), maximizer(X), nl,write('maximizer algo '), write(X), coupAJouerMaximizer(L,R,Value),!.
